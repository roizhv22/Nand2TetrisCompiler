...
def assemble_file(
    ...
    generator = Code()
    n_1 = 0
    while first_pass.has_more_commands() == True:
        if first_pass.command_type() == "L_COMMAND":
            symbol_table.add_entry(first_pass.symbol(), n_1)
        else:
            n_1+=1
        first_pass.advance()
    input_file.seek(0)
    second_pass = Parser(input_file)
    n_2 = 16
    instruction = ""
    while second_pass.has_more_commands() == True:
        if second_pass.command_type() == "A_COMMAND":
            sym = second_pass.symbol()
            if not sym.isdigit():
                if symbol_table.contains(sym):
                    instruction = format(symbol_table.get_address(sym), '016b')
                else:
                    symbol_table.add_entry(sym, n_2)
                    n_2+=1
                    instruction = format(symbol_table.get_address(sym), '016b')
            else:
                instruction = format(int(sym), '016b')
        elif second_pass.command_type() == "C_COMMAND":
            instruction = "1" + generator.comp(second_pass.comp()) + \
                          generator.dest(second_pass.dest()) + \
                          generator.jump(second_pass.jump())
        elif second_pass.command_type() == "L_COMMAND":
            second_pass.advance()
            continue
        second_pass.advance()
